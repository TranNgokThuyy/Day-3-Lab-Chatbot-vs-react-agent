...
```bash
cd Day-3-Lab-Chatbot-vs-react-agent-tam
...
## 🖥️ UI/UX Demo
A simple browser UI is available with Streamlit to compare the Chatbot baseline and the ReAct agent.
...
Features:
- Chọn giữa `Mock`, `OpenAI`, `Gemini`, hoặc `Local` provider
- Chuyển đổi giữa `Baseline Chatbot` và `ReAct Agent`
- Gửi truy vấn và xem phản hồi trực tiếp
- Hiển thị mô tả tool `search_documents` cho ReAct Agent
